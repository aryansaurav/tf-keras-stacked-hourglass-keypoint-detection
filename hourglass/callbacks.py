...
from eval import eval_PCK, eval_PCK_new
...
class EvalCallBackNew(Callback):
    def __init__(self, log_dir, validation_dataset, class_names, model_input_shape, model_type):
        self.log_dir = log_dir
        ...
        with open(os.path.join(self.log_dir, 'val.txt'), 'w+') as xfile:
            # xfile.write('model:' + model_type + ';dataset:' + self.eval_dataset.get_dataset_name() + '\n')
            xfile.write('model:' + model_type + '\n')
        xfile.close()

    def on_epoch_end(self, epoch, logs=None):
        # val_acc, _ = eval_PCK(self.model, 'H5', self.validation_dataset, self.class_names, self.model_input_shape, score_threshold=0.5, normalize=self.normalize, conf_threshold=1e-6, save_result=False)
        val_acc, _ = eval_PCK_new(self.model, 'H5', self.validation_dataset, self.class_names, self.model_input_shape, score_threshold=0.5, normalize=self.normalize, conf_threshold=1e-6, save_result=False)
